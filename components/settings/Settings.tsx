...
import { User, Bell, Palette, Shield, Database, ChevronRight } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-8" style={{ maxWidth: '640px' }}>
        {activeSection === 'profile' && (
          <Section title="Profil" description="Vos informations professionnelles">
            <Field label="Prénom">
              <input style={inputStyle} value={profile.firstName} onChange={e => setProfile(p => ({ ...p, firstName: e.target.value }))} placeholder="Jean" />
            </Field>
            <Field label="Nom">
              <input style={inputStyle} value={profile.lastName} onChange={e => setProfile(p => ({ ...p, lastName: e.target.value }))} placeholder="Dupont" />
            </Field>
            <Field label="Email">
              <input style={inputStyle} type="email" value={profile.email} onChange={e => setProfile(p => ({ ...p, email: e.target.value }))} placeholder="jean.dupont@cabinet.fr" />
            </Field>
            <Field label="Barreau">
              <input style={inputStyle} value={profile.barreau} onChange={e => setProfile(p => ({ ...p, barreau: e.target.value }))} placeholder="Paris" />
            </Field>
            <Field label="Cabinet">
              <input style={inputStyle} value={profile.cabinet} onChange={e => setProfile(p => ({ ...p, cabinet: e.target.value }))} placeholder="Cabinet Dupont & Associés" />
            </Field>
            <Field label="Téléphone">
              <input style={inputStyle} value={profile.phone} onChange={e => setProfile(p => ({ ...p, phone: e.target.value }))} placeholder="+33 1 00 00 00 00" />
            </Field>
          </Section>
        )}
        ...
        {activeSection === 'notifications' && (
          <Section title="Notifications" description="Gérez vos préférences de notifications">
            ...
            <Toggle
              ...
              description="Soyez prévenu avant chaque échéance"
              checked={notifs.deadlineReminders}
              onChange={v => setNotifs(n => ({ ...n, deadlineReminders: v }))}
            />
            <Toggle
              label="Nouveaux documents"
              description="Notification lors de l'ajout de documents"
              checked={notifs.newDocuments}
              onChange={v => setNotifs(n => ({ ...n, newDocuments: v }))}
            />
            <Toggle
        ...
        {activeSection === 'security' && (
          <Section title="Sécurité" description="Protégez votre compte">
            <div style={{ padding: '16px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', marginBottom: '12px' }}>
        ...
        {activeSection === 'data' && (
          <Section title="Données" description="Exportez ou supprimez vos données">
            ...
            <div style={{ padding: '16px', background: 'var(--color-surface)', border: '1px solid var(--color-error)', borderRadius: 'var(--radius-md)' }}>
